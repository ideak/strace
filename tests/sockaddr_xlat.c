...
check_mctp(void)
{
	static const struct {
		...
		{ { AF_MCTP, .smctp_network=MCTP_NET_ANY,
		...
		{ { AF_MCTP, .smctp_network=-1234567890,
		    ...
		    .smctp_type=1, .smctp_tag=8 },
		  "{sa_family=" XLAT_KNOWN(0x2d, "AF_MCTP")
		  ", smctp_network=-1234567890"
		  ", smctp_addr={s_addr=" XLAT_KNOWN(0xff, "MCTP_ADDR_ANY") "}"
		  ", smctp_type=0x1, smctp_tag=0x8}" },
		{ { AF_MCTP, .smctp_network=2134567890,
		    .smctp_addr={ .s_addr = 0x42 },
		    .smctp_type=0x23, .smctp_tag=0x69 },
		  "{sa_family=" XLAT_KNOWN(0x2d, "AF_MCTP")
		  ", smctp_network=2134567890, smctp_addr={s_addr=0x42}"
		  ", smctp_type=0x23, smctp_tag=0x69}" },
	};
	...
	printf("connect(-1, {sa_family=" XLAT_KNOWN(0x2d, "AF_MCTP")
	       ", smctp_network=%d, smctp_addr={s_addr=%#hhx}"
		  ", smctp_type=%#hhx, smctp_tag=%#hhx}, %zu) = %s\n",
	       sa_mctp->smctp_network, sa_mctp->smctp_addr.s_addr,
	       sa_mctp->smctp_type, sa_mctp->smctp_tag, sizeof(*sa_mctp),
	       sprintrc(rc));
